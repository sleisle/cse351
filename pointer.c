 ...
 */
int intSize() {
  /* Returns the size of an integer in bytes. Casts to int
      cause compiler warning, this is ok in this case*/
  int intArray[10];
  int * intPtr1;
  int * intPtr2;
 ...
 */
int doubleSize() {
  /* Returns the size of a double in bytes. Casts to int
      cause compiler warning, this is ok in this case */
  double doubArray[10];
  double * doubPtr1;
  double * doubPtr2;
  ...
  int ret = (int)doubPtr2 - (int)doubPtr1;
...
int pointerSize() {
  ...
  ptrPtr1 = &ptrArray[0];
  ptrPtr2 = &ptrArray[1];
  int ret = (int)ptrPtr2 - (int)ptrPtr1;
 ...
 */
int changeValue() {
  /* Changes the sixth element of an array using specified constraints */
  int intArray[10];
...
int withinSameBlock(int *ptr1, int *ptr2) {
  /* Checks the seventh bit for equality to see if two pointers are in
      the same 64-byte block */
  int byte1 = (int)ptr1 >> 6;
  int byte2 = (int)ptr2 >> 6;
...
int withinArray(int * intArray, int size, int * ptr) {
  /* Finds out if a pointer ptr points to memory allocated for array
      intArray */
  int * max = intArray + size;

  int check = (max - ptr) - (int)intArray;
